...
use crate::rendering::{Camera, Display};
    ...
    pub fn update(&mut self, ctx: &egui::CtxRef, display: &Display, simulation: &Simulation) {
        ...
        egui::SidePanel::left("Debug Info")
            .show(ctx, |ui| {
                ...
                ui.heading("Camera");
                ui.label(format!("Translation: ({:.2}, {:.2})", cam.translation.x, cam.translation.y));
                ui.label(format!("Window Size: ({:.2}, {:.2})", cam.window_size.x, cam.window_size.y));
