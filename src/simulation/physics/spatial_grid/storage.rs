...
impl<T: Default> DenseStorage<T> {
    ...
    pub fn pos(&self, (x, y): CellIdx) -> usize {
    ...
    pub fn cell_mut(&mut self, pos: Vec2) -> (CellIdx, &mut T) {
        ...
        if y <= down {
            ...
            self.height += padup;
            if !reallocate {
                self.cells.resize_with((self.width * self.height) as usize, T::default);
            }
        }
        ...
        if reallocate {
            ...
            for y in 0..oldh {
                ...
                newvec.extend((0..oldw).map(|x| std::mem::take(self.cells.get_mut((y * oldw + x) as usize).unwrap())));
