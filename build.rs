...
fn main() {
    ...
    let vs = include_str!("src/particles.vert");
    ...
    fs::write(
        &Path::new(&manifest_dir).join("src/particles.vert.spv"),
        vs_bytes,
    )
    .unwrap();

    let fs = include_str!("src/particles.frag");
    ...
    fs::write(
        &Path::new(&manifest_dir).join("src/particles.frag.spv"),
        fs_bytes,
    )
    .unwrap();
